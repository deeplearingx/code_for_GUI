...
class Agent(BaseAgent):
    ...
    def _fallback(self, input_data: AgentInput) -> tuple[str, dict]:
        ...
        # 2. Immediate post-open fallback
        popup_confidence = float(input_data.extra.get("popup_confidence", 0.0))
        early_action = decide_post_open_action(
            self._task,
            step_count,
        ...
        if early_action is not None:
            action, parameters, _reason, _confidence = early_action
            self._fallback_ready_to_type = False
        ...
        if self._task and self._task.has_pending_text():
            ...
            expected_type_click = _points_close(last_click_point, get_search_bar_coord(app_name)) or (
                self._task.task_type in FLOW_TASK_TYPES
                and _points_close(last_click_point, get_flow_continue_coord(app_name))
            )
            ready_for_type = (
                self._fallback_ready_to_type
                and last_action == ACTION_CLICK
                and pending_text is not None
                and expected_type_click
            )
            if ready_for_type:
                self._fallback_ready_to_type = False
                return (ACTION_TYPE, {"text": pending_text})
